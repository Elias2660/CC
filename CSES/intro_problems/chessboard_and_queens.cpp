...
void find_combinations(int queens, int row, vector<int> cols_left,
                       vector<int> diags, vector<int> cross_diags,
                       vector<vector<char> > left_chessboard) {
  if (queens == 8) {
    combinations++;
    return;
  }
  for (int col : cols_left) {
    if (chessboard[row][col] != '*' &&
        ...
        !(find(cross_diags.begin(), cross_diags.end(), col + row) !=
          cross_diags.end())) {
      vector<int> new_cross_diags = cross_diags, new_diags = diags,
                  new_cols_left = cols_left;
      ...
      new_cols_left.erase(
          remove(new_cols_left.begin(), new_cols_left.end(), col),
          new_cols_left.end());
      find_combinations(queens + 1, row + 1, new_cols_left, new_diags,
                        new_cross_diags, new_chessboard);
    }
  }
}
