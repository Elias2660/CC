...
// actually not...
// there could actually be a recursive solution, where at a certain position,
// one could do all possible actions this will run a recursive algorithm with
// optimizations
...
vector<char> find_possible_moves(vector<vector<int> > cur_maze,
                                 vector<int> cur_pos) {
...
void find_combinations(vector<vector<int> > cur_maze, vector<int> cur_pos,
                       int move_num, string path) {
  ...
  } else if (find(possible_moves.begin(), possible_moves.end(), path_move) ==
             possible_moves.end()) {
  }
}
